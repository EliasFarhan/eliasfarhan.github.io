---
layout: home
---

![Photo by [Yves Leresche](https://www.yvesleresche.ch/)](/images/elias_farhan.jpg){: style="display: block;
  margin-left: auto;
  margin-right: auto;
  width: 30%;"}

Hi! I'm Elias Farhan, an indie game programmer from Switzerland. I mostly do C++ and C#, teach some games programming, work on game engines and develop some silly games.

- Head Instructor of the Games Programming Department at [SAE Institute Geneva](https://geneva.sae.edu/).
- Co-founder of indie game studio [Team KwaKwa](https://team-kwakwa.com/), based in Lausanne, Switzerland.
- Co-organizer of the [Game Developers Suisse Romande Meetup](https://www.meetup.com/Game-Developers-Suisse-Romande/).

## Games

Beach Slap, Splash Blast Panic, Soup Raiders and a decade of game jam entries are all on the [games page](/games/).

## Projects

CompGraphEditor, SplashOnline, NekoPhysics, NekoLib and more, over on the [projects page](/projects/).

## Talks & CV

I speak about rollback netcode and optimisation at conferences like CppCon 2024 and ADDON. Slides and videos are on the [talks page](/talks/). My [CV](/cv/) has the longer story.

## Blog

I write about graphics programming, game engines, C++, teaching and travel. Browse [everything](/blog/), or dive into
[gamedev](/category/gamedev/),
[graphics](/category/graphics/),
[c++](/category/cpp/),
[thoughts](/category/thoughts/) or
[travel](/category/travel/).
